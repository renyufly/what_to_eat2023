...
class dining_user_con:
    ...
    def initial_add_dished(self):
        ...
        with open("data_initial.txt", "r", encoding="utf-8") as file:
            for line in file:
                ...
                self.cur.execute(
                    """SELECT id FROM cafeteria WHERE name = ?""", (cafename,)
                )
                ...
                if cafe_id:
                    ...
                    self.cur.execute(
                        """INSERT INTO cafeteria(name) VALUES (?)""", (cafename,)
                    )
                    self.con.commit()
                    self.cur.execute(
                        """SELECT id FROM cafeteria WHERE name = ?""", (cafename,)
                ...
                self.cur.execute(
                    """SELECT id FROM food_counter WHERE name = ? AND cafeteria_id = ?""",
                ...
                self.cur.execute(
                    """INSERT INTO dishes(name, counter_id, number, img_path) VALUES(?,?,?,?)""",
                    (dishname, counter_id[0][0], 0, imgpath),
                )
                self.con.commit()
    ...
    def showCafeAllCounterName(self, cafename: str) -> list:
        ...
        self.cur.execute(
            """SELECT name FROM food_counter WHERE cafeteria_id = ?""", cafe_id
        )
    ...
    def showCounterAllDish(self, cafename: str, countername: str) -> list:
        ...
        cafe_id = self.cur.fetchall()[0]
        self.cur.execute(
            """SELECT id FROM food_counter WHERE cafeteria_id = ? AND name = ?""",
    ...
    def showOneDish(self, cafename: str, countername: str, dishname: str) -> list:
        ...
        counter_id = self.cur.fetchall()[0]
        self.cur.execute(
            """SELECT * FROM dishes WHERE counter_id = ? AND name = ?""",
            (counter_id[0], dishname),
        )
        dish = self.cur.fetchall()
        return dish
    ...
    def updateCafeName(self, cafename: str, newname: str):
        ...
        cafe_id = self.cur.fetchall()[0]
        self.cur.execute(
            "UPDATE cafeteria SET name = ? WHERE id = ?", (newname, cafe_id[0])
        )
        self.con.commit()
    ...
    def updateCounterName(self, cafename: str, countername: str, newname: str):
        ...
        counter_id = self.cur.fetchall()[0]
        self.cur.execute(
            "UPDATE food_counter SET name = ? WHERE id = ?", (newname, counter_id[0])
        )
        self.con.commit()

    def updateDishName(
        self, cafename: str, countername: str, dishname: str, newname: str
    ):
        """更新菜名(其他关联信息不变)"""
        self.cur.execute("""SELECT id FROM cafeteria WHERE name = ?""", (cafename,))
        cafe_id = self.cur.fetchall()[0]
        ...
        counter_id = self.cur.fetchall()[0]
        self.cur.execute(
            """SELECT id FROM dishes WHERE counter_id = ? AND name = ?""",
            (counter_id[0], dishname),
        )
        dish_id = self.cur.fetchall()[0]
        self.cur.execute(
            """UPDATE dishes SET name = ? WHERE id = ?""", (newname, dish_id[0])
        )
        self.con.commit()
    ...
    def increaseNumber(self, cafename: str, countername: str, dishname: str):
        ...
        num = num + 1
        self.cur.execute(
            """UPDATE dishes SET number = ? WHERE id = ?""", (num, dish_id[0])
        )
        self.con.commit()
    ...
    def showAllDishRankingList(self):
        ...
        for item in sorted_list:
            self.cur.execute(
                """SELECT name, cafeteria_id FROM food_counter WHERE id = ?""",
                (item[2],),
            )
    ...
    def recommendDish(self, username: str) -> list:
        """提供基于⼀天中的时间、⽤⼾⼝味偏好以及早餐、午餐和晚餐的膳⻝历史记录的膳⻝推荐"""
        """菜名 柜台名 食堂名"""
        import datetime
        import random

        # from userRecordDatabase import showAllDishName
        ...
        """拉取用户已有的用餐记录"""
        # history_list = showAllDishName(username)
        """拉取全部菜品"""
        self.cur.execute("""SELECT * FROM dishes""")
        all_dishes_list = self.cur.fetchall()
        """推荐算法"""
        dish_list = []
        for itemTuple in all_dishes_list:
            self.cur.execute(
                """SELECT name, cafeteria_id FROM food_counter WHERE id = ?""",
                (itemTuple[2],),
            )
            countername, cafe_id = self.cur.fetchall()[0]
            if (meal == "早餐") ^ (countername == "早餐柜台"):
                continue
            self.cur.execute("""SELECT name FROM cafeteria WHERE id = ?""", (cafe_id,))
            cafename = self.cur.fetchall()[0]
            dish_list.append(itemTuple[1] + " " + countername + " " + cafename[0])
        random.shuffle(dish_list)
        return dish_list[:10]
